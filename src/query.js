        ...
        this.api.cachedGet(cubesUrl, 'json').then((r) => {
          ...
          for (let i = 0; i < cubes.length; i++) {
            ...
            if (dl.downgradeLevel) {
              ...
              const downgradeParentIndex = `${Math.floor(cubeContainer.cube.x / factor.x)}_${Math.floor(cubeContainer.cube.y / factor.y)}_${Math.floor(cubeContainer.cube.z / factor.z)}`;
          ...
          if (dlIndex === 0) {
            for (let i = 0; i < clonedThingsThatNeedElevation.length; i++) {
              ...
              if (this.loader.pyrite.modelConfig.markerYOffsets) {
                const translateYValue = this.loader.pyrite.modelConfig.markerYOffsets[clonedThingsThatNeedElevation[i].TeeId || clonedThingsThatNeedElevation[i].HoleId];
                if (translateYValue) {
